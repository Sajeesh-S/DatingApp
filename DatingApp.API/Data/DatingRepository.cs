...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<PagedList<User>> GetUsers(UserParams userParams)
        {
            ...
            if (userParams.MinAge != 18 || userParams.MaxAg != 99)
            {
                var minDob = DateTime.Today.AddYears(-userParams.MaxAg - 1);
                var maxDob = DateTime.Today.AddYears(-userParams.MinAge);
                users = users.Where(users => users.DateoOfBirth >= minDob && users.DateoOfBirth <= maxDob);
            }
        ...
        {
            var user = await _context.Users.Include(x => x.Likees).Include(x => x.Likees)
